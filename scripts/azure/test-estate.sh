...
JSON
az monitor data-collection rule create -g "$RG" -n "$DCR" --rule-file "$DCR_FILE" -o none
...
az monitor scheduled-query create -g "$RG" -n "pulse-synextra-test-vm-os-disk-free" \
  ...
  --condition "avg FreePct < 90 resource id _ResourceId" \
  --condition-query FreePct='InsightsMetrics | where Origin == "vm.azm.ms" and Namespace == "LogicalDisk" and Name == "FreeSpacePercentage" | extend Mount = tostring(todynamic(Tags)["vm.azm.ms/mountId"]) | where Mount == "C:" | summarize FreePct = avg(Val) by bin(TimeGenerated, 5m), _ResourceId' \
